...
#
codes <- read.xlsx('../data/other_files/codes/Excel codelists/MRC.xlsx',1) # load codes
...
copd_eligible <- merge(copd_eligible,fev1_before[,c('patid','airflow')],by='patid',all.x=T)
...
imd <- read.table('../data/Final + new linkage/linkage_download_jan2018/patient_imd2010_16_276R.txt',sep='\t',header=T)
copd_eligible$imd2010_20 <- NULL # remove old IMD data
copd_eligible <- merge(copd_eligible,imd[,-2],by='patid',all.x=T)


#smoking, make an exception, use memory instead of hard drive to avoid strange error

# find instances of medcodes
codes <- read.dta13('../data/other_files/codes/smoking_codes.dta')[,c(1,3)]
...
clinical_relevant2 <- as.data.table.ffdf(clinical_before[clinical_before$enttype == 4,c('patid','adid','eventdate','first_copd')])
additional_relevant <- as.data.table.ffdf(additional_eligible[additional_eligible$enttype==4,c('patid','adid','data1')])
...
clinical_relevant <- clinical_before[clinical_before$enttype == 13,c('patid','eventdate','adid','first_copd','enttype')]
...
ord <- ffdforder(clinical_relevant[c('patid','before_diagnosis')]) 
clinical_relevant <- clinical_relevant[ord,] 
...
additional_relevant <- additional_eligible[additional_eligible$enttype==13,c('patid','adid','data3')]
...
missing <- is.na(copd_eligible$bm)
...
# find egfr data
egfr <- test_before[test_before$enttype==466,c('patid','eventdate','data1','data2')] 
...
# sort by patid, then how many days before diagnosis, efficiently using ffdf
ord <- ffdforder(egfr[c('patid','before_diagnosis')]) 
egfr <- egfr[ord,] 
...
# find instances
codes <- read.dta13('../data/other_files/codes/gord.dta')
relevant <- as.data.table.ffdf(find_medcodes(codes))
...
tmp <- vapply(copd_eligible[,33:56],is.na,numeric(dim(copd_eligible)[1]))
...
save(copd_eligible,file='../output/proc_data/2_3.RData')
...
therapy_relevant <- therapy_before[therapy_before$prodcode %in% prods$prodcode,c('patid','eventdate','first_copd')]
...
codes <- read.dta13('../data/other_files/codes/asthma.dta')
...
therapy_relevant <- therapy_before[therapy_before$prodcode %in% epi$prodcode,c('patid','eventdate','first_copd')]
therapy_relevant$before_diagnosis <- therapy_relevant$first_copd - therapy_relevant$eventdate
therapy_relevant <- as.data.table.ffdf(therapy_relevant[therapy_relevant$before_diagnosis <= 365,])
...
therapy_relevant2 <- therapy_before[therapy_before$prodcode %in% prods$prodcode,c('patid','eventdate','first_copd')]
...
rm(all_copd,both,codes,eligible,epi,imd,ind,linked,missing,ord,prods,relevant,rels,therapy_relevant,therapy_relevant2)
