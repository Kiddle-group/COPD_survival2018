...
print('..patient info')
all_copd <- read.dta13("../data/first_copd_222790.dta") # First COPD diagnosis for each patient, from Quint group
patient <- read.dta13('../data/pat_pract_222790.dta') # data on patient demographics, from Quint group
...
le <- read.table(file="../data/linkage_eligibility.txt",sep="\t",header=T) # check ONS and HES linkage eligibility
